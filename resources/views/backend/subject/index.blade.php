...
@section('content')
    ...
    <nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>
        学科模块管理 <span class="c-gray en">&gt;</span> 学科列表
	...
	<a class="btn btn-primary radius" href="javascript:;" onclick="subject_add('添加学科','{{ url('System/Subject/Add') }}','800')">
	<i class="Hui-iconfont">&#xe600;</i> 添加学科
	</a>
		</span>
        </div>
    ...
    <script>
        ...
        $('.list-data').DataTable({
            ...
            "createdRow": function(row,data,index){
                ...
                if(data.logo){
                    $(row).children().eq(3).html('<img src="/uploads/'+data.logo+'" style="width: 60px;height: 60px;">');
                }else{
                    $(row).children().eq(3).html('暂无logo图片');
                }
                $(row).children().eq(-1).html(`<a title="编辑" href="javascript:;"
                onclick="subject_edit('编辑学科信息','{{url('/System/Subject/Edit')}}/`+ data.id +`' ,'`+ data.id +`')"
