...
@section('content')
    <article class="page-container">
        <form class="form form-horizontal" id="form-admin-add" action="{{ url('System/Course/Store') }}" method="post">
            {{csrf_field()}}
            <div class="row cl">
		...
		<select class="select" name="profession_id" size="1">
			@foreach($profession as $pro)
                <option value="{{$pro['id']}}">{{$pro['profession_name']}}</option>
            @endforeach
		</select>
		</span> </div>
            </div>
            <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9">
                    <input type="text" class="input-text" value="" placeholder="" id="teacher_id" name="teacher_id">
                </div>
            </div>
            <div class="row cl">
                <label class="form-label col-xs-4 col-sm-3">课程简介：</label>
                <div class="formControls col-xs-8 col-sm-9">
                    <textarea name="course_desc" cols="" rows="" class="textarea"  placeholder="专业详情介绍" dragonfly="true" onKeyUp="$.Huitextarealength(this,100)"></textarea>
